...
void InitParticle(int pause) {
    for (int i = 0; i < MAX_PARTICLES; i++) {
        ...
        particles[i].veloc_y = 0.1f + ((float)(rand() % 10) / 1000.0f); // Pequena variação na queda
...
void drawGLScene(GLFWwindow* window) {
    ...
    for (int i = 0; i < MAX_PARTICLES; i++) {
        if (particles[i].lifetime > 0) {
            ...
            // Atualiza a posição com velocidade e gravidade
            particles[i].y -= particles[i].veloc_y;
            particles[i].y -= GRAVITY;
            particles[i].lifetime--;

            // Se sair da tela inferior, reinicia a partícula
            ...
            glVertex2f(particles[i].x, particles[i].y + 0.2f);
        }
    }
    glEnd();
